...
__all__ = ['AutogradLinearSolver', 'LinearSolver', 'SparseLinearSolver', 'PDESolver', 'FDMDerivatives',
           'FDMAdjointDerivatives', 'FDMAssembly', 'UnpaddedFDM', 'FDM', 'configure_legion_memory']

# Cell
def configure_legion_memory(replheap_size_mb=64):
    """Configure Legion/Legate memory settings to avoid replheap exhaustion
    
    Args:
        replheap_size_mb: Size of replicated heap in MB (default: 64)
    """
    import os
    # Set Legion replicated heap size (avoiding the FATAL error)
    os.environ["LEGION_REPLHEAP_SIZE"] = str(replheap_size_mb * 1024 * 1024)
    # Optional: Set global memory size if needed
    # os.environ["LEGION_GLOBAL_MEM_SIZE"] = "8GB"
    print(f"Legion memory configured with replheap size: {replheap_size_mb}MB")
...
try:
    # Configure Legion memory before importing legate
    configure_legion_memory(replheap_size_mb=128)  # Set to 128MB to avoid replheap exhaustion
    ...
    if hasattr(ls, 'set_resource_limit'):
        ls.set_resource_limit('replicated', 128 * 1024 * 1024)  # 128MB
    if hasattr(cn, 'set_resource_limit'):
        cn.set_resource_limit('replicated', 128 * 1024 * 1024)  # 128MB
...
class SparseLinearSolver(LinearSolver):
    ...
    """
    def __init__(self,
                 optimizer: str = "cupy", # "scipy", "cupy", "legate", "amgx"
    ...
    def _solver(self):
        ...
        if self.optimizer == "amgx" and self.have_pyamgx:
            ...
            def solve_gpu_amgx(A, b, phase='forward', θ_current=None):
                ...
                #             amgx_config = {
                # "config_version": 2,
                # "determinism_flag": 1,  # Ensures reproducible results
                # "solver": {
                #     "preconditioner": {
                #         "algorithm": "JACOBI",  # Similar to CuPy's diagonal preconditioner
                #         "jacobi_weight": 1.0    # No damping (similar to CuPy's direct inverse)
                #     },
                #     "solver": "PCG",            # Preconditioned Conjugate Gradient
                #     "max_iters": self.cg_max_iter or 1000,  # Match CuPy's max iterations
                #     "tolerance": self.cg_tol,   # Match CuPy's tolerance
                #     "norm": "L2",               # L2 norm for convergence check
                #     "monitor_residual": 1,      # Monitor convergence like CuPy's callback
                #     "store_res_history": 1,     # Store convergence history
                #     "convergence": "RELATIVE_INI_CORE"  # Similar to CuPy's convergence criteria
                # },
                # "matrix": {
        ...
        if self.optimizer == "legate" and self.have_legate_sparse:
            ...
            from legate_sparse.linalg import cg as ls_cg, gmres as ls_gmres, LinearOperator as ls_LinearOperator
            def solve_gpu_legate(A, b, phase='forward', θ_current=None):
                try:
                    ...
                    if self.preconditioner == 'jacobi':
                        diag = A_ls.diagonal()
                        inv_diag = cn.where(diag != 0, 1.0 / diag, 1.0)
                        inv_diag_cn = inv_diag
                        def mv(x):
                    ...
                    result = np.asarray(x_cn)
                    return result
                finally:
                    # Explicitly clean up Legion/Legate resources to prevent replheap exhaustion
                    if 'data_cn' in locals(): del data_cn
                    if 'indices_cn' in locals(): del indices_cn
                    if 'indptr_cn' in locals(): del indptr_cn
                    if 'A_ls' in locals(): del A_ls
                    if 'b_cn' in locals(): del b_cn
                    if 'diag' in locals(): del diag
                    if 'inv_diag' in locals(): del inv_diag
                    if 'inv_diag_cn' in locals(): del inv_diag_cn
                    if 'x_cn' in locals(): del x_cn
                    
                    # Force memory release for Legion/Legate
                    if 'cn' in locals() and cn is not None and hasattr(cn, 'flush_resources'):
                        cn.flush_resources()
